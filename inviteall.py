import aminofixfix as aminofix
import time
from concurrent.futures import ThreadPoolExecutor

def authenticate(client: aminofix.Client):
    while True:
        try:
...
def list_communities(client: aminofix.Client):
    while True:
        try:
...
def list_chats(sub_client: aminofix.SubClient):
    while True:
        try:
...
def invite_online_users(sub_client: aminofix.SubClient):
...
def invite_recent_users(sub_client: aminofix.SubClient):
    chat_id = list_chats(sub_client)
    while True:
...
def invite_user_followers(client: aminofix.Client, sub_client: aminofix.SubClient):
    chat_id = list_chats(sub_client)
...
""")
    ...
    sub_client = aminofix.SubClient(comId=com_id, mainClient=client.client)
